...
page: **75,091 bytes** uncompressed — 20.6 KB markup, 23.9 KB stylesheet,
...
numbers page: the difference is 5,263 bytes of stylesheet and 55 bytes of
markup, and it buys motion that needs no script.
...
the
numbers tiles no longer borrow their evidence from the other repository's
README: they used to link there, and a README that repeats a number does not
prove it. It is also why this repository is now linked from the site — the
build figure in every footer is measured here, so the file behind it has to be
reachable.
...
**the one place to change when the custom domain arrives.** `REPO` sits beside
it and is the one place to change if the snapshot repository ever moves;
`SELBST`, a few lines below, is the same for this website's own repository —
the file behind the build figure in every footer.
...
One thing no gate here can check: the two repository links. `check_links`
skips external targets by design, so if either repository is renamed, moved or
made private, every check stays green while the site links into nothing — and
the claim on `/numbers/` that exactly one figure cannot be checked would
quietly stop being true. Both live in `bake.py` as `REPO` and `SELBST`; open
them by hand after any change to either repository.
