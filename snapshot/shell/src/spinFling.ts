...
export function initSpinFling(opts: SpinFlingOptions): () => void {
  ...
  const reset = (b: Body) => {
    b.el.classList.remove("spinlift");   // Greif-Glow weg; Snap-Zurück via Inline-Transition
  ...
  const onMove = (e: PointerEvent) => {
    ...
    if (!body.lifted && Math.hypot(e.clientX - body.startX, e.clientY - body.startY) > LIFT_THRESH) {
      body.lifted = true;
      body.el.classList.add("spinlift");   // Greif-Glow (docs/06 §6.3) — Klasse, kein Inline-Style
  ...
  const collideFloat = (b: Body) => {
    ...
    // VERBATIM aus der kanonischen spinfling.js (sfCollide): Schleife ueber ALLE Floats
    // des Selektors, exaktes gedrehtes Panel-Rechteck, Kontakt-Oberflaechen-Geschwindigkeit.
    const c = Math.cos(-b.th), s = Math.sin(-b.th), hw = b.w / 2, hh = b.h / 2;
    for (const f of document.querySelectorAll<HTMLElement>(opts.floatSelector)) {
      ...
      if (Math.hypot(ux, uy) < KICK_MIN) continue;           // sanfte Beruehrung = kein Kick
      f.dispatchEvent(new CustomEvent("dizzi:fling",
        { detail: { vx: (ux / 60) * TRANSFER, vy: (uy / 60) * TRANSFER } }));   // px/Frame
      b.cool = 12; return;
    }
  };
