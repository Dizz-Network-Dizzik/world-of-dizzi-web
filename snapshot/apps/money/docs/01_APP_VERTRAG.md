# App-Vertrag — Dizz Money — Finanzmanagement
...
1. **Eigener Core-Service** (FastAPI), Port-Vorschlag **8210** (lokal, localhost-gebunden).
2. **Read-only Stats-Endpoint** `/api/summary` → liefert die Kennzahlen für Dizzis Dashboard-Kachel
   `finanzen` (Muster: Trading Bot `:8137/api/summary`).
