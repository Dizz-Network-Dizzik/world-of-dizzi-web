# App-Vertrag — Dizz Creating — Creator
...
1. **Eigener Core-Service** (FastAPI), Port-Vorschlag **8214** (lokal, localhost-gebunden).
2. **Read-only Stats-Endpoint** `/api/summary` → liefert die Kennzahlen für Dizzis Dashboard-Kachel
   `creator` (Muster: Trading Bot `:8137/api/summary`).
