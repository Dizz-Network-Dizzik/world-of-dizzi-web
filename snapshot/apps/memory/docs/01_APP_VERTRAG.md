# App-Vertrag — Dizz Memory (Wissensspeicher) — Ordner `archiv`

> **Stand 13.06.2026:** Vertrag ERFÜLLT (Bau aus `templates/refapp`, appkit 1.6.0 vendort).
> Manifest-id `memory`, Marke „Dizz Memory" / Funktion „Wissensspeicher", **Port 8212**,
> sensitivity `hoch`. Konformität maschinell grün (`appkit.conformance.check_contract`).
...
1. **Eigener Core-Service** (FastAPI), Port **8212** (lokal, localhost-gebunden). ✅ GEBAUT.
2. **Read-only Stats-Endpoint** `/api/summary` → liefert die Kennzahlen für Dizzis Dashboard-Kachel
   `memory` (Notizen/Ordner/Labels/Zuletzt; Muster: Trading Bot `:8137/api/summary`). ✅ GEBAUT.
