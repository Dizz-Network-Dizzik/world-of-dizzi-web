...
(function (global) {
  ...
  function initSpinFling(opts) {
    ...
    function sfGridScrolls() { try { const cs = getComputedStyle(grid);
      return /(auto|scroll)/.test(cs.overflowY) || /(auto|scroll)/.test(cs.overflowX); } catch (e) { return false; } }
    function sfLock() { if (SUNLK) { clearTimeout(SUNLK); SUNLK = 0; } if (SOVF) return;
      const lockGrid = sfGridScrolls();
      SOVF = [document.documentElement.style.overflow, document.body.style.overflow, lockGrid ? grid.style.overflow : null];
      document.documentElement.style.overflow = 'hidden'; document.body.style.overflow = 'hidden';
      if (lockGrid) grid.style.overflow = 'hidden'; }
    function sfUnlock() { SUNLK = 0; if (!SOVF) return;
      document.documentElement.style.overflow = SOVF[0]; document.body.style.overflow = SOVF[1];
      if (SOVF[2] !== null) grid.style.overflow = SOVF[2]; SOVF = null; }
    function sfMove(e) {
